...
class NeVAWrapper(nn.Module):
    def __init__(self, downstream_model, criterion, target_function, image_size, foveation_sigma, blur_filter_size, blur_sigma, forgetting, foveation_aggregation=1, device="cpu"):
    ...
    def forward(self, x, foveation_positions):
        ...
        #using audioclip model for encoding images
        ((_, output, _), _), _  = self.downstream_model(image = x + applied_blur)

        return output

    def initialize_scanpath_generation(self, x, batch_size):
        self.internal_representation = torch.zeros((batch_size, 1, self.image_size, self.image_size), device='cpu')
    ...
    def run_optimization(self, x, labels, scanpath_length, opt_iterations, learning_rate, filename, random_restarts=False):
        x = x.unsqueeze(0) #torch.Size([1, 3, 224, 224])
        batch_size = x.size(0)
        ...
        for step in range(scanpath_length):
            foveation_pos = torch.zeros((batch_size, 2, 1, 1), device='cpu', requires_grad=True)
            best_foveation_pos = torch.zeros((batch_size, 2, 1, 1), device='cpu')
            best_loss = torch.ones((batch_size), device='cpu', dtype=torch.float16) * float("inf")
            ...
            for _ in range(opt_iterations):
                ...
                # save best
                idxs = loss < best_loss
                best_loss[idxs] = loss[idxs].to(torch.float16)
        ...
        # apply blur on image
        current_foveation_area = self.internal_representation + current_foveation_mask
        blurring_mask = torch.clip(self.ones - current_foveation_area, 0, 1)
        applied_blur = self.blur * blurring_mask
        combine_img = x + applied_blur

        # save image with blur
        combine_img = combine_img.squeeze(0).permute(1, 2, 0).detach().numpy()
        imageRGB = cv2.cvtColor(combine_img, cv2.COLOR_BGR2RGB)
        img_rgb = Image.fromarray((imageRGB*255).astype('uint8'))
        img_rgb.save("output/{}.png".format(filename))

        return torch.stack(scanpath, 1).squeeze(), torch.stack(loss_history, 1).squeeze(), scanpath
...
def calculate_blur(images, blur_filter_size, sigma=5):
    ...
    window = create_window(blur_filter_size, 3, sigma)
